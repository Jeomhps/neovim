-- Boots the nixInfo shim and the lze lazy-loading engine.
-- Must be require()'d as the very first thing in init.lua (right after
-- vim.loader.enable()), before anything else tries to require('lze').
--
-- Returns { ready = false } if init.lua should stop immediately (fresh
-- non-Nix install, plugins just got downloaded and need a restart).
